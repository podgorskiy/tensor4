...
{
	t4::tensor4f x33 = t4::Conv2d<3, 3, 1, 1, 1, 1, 1, 1>(xinput_1, ctx.features_0_weight, ctx.features_0_bias); //features.0
	t4::release(xinput_1);
	t4::tensor4f x34 = t4::ReluInplace(x33); //features.1
	t4::release(x33);
	t4::tensor4f x35 = t4::Conv2d<3, 3, 1, 1, 1, 1, 1, 1>(x34, ctx.features_2_weight, ctx.features_2_bias); //features.2
	t4::release(x34);
	t4::tensor4f x36 = t4::ReluInplace(x35); //features.3
	t4::release(x35);
	...
	t4::release(x56);
	t4::tensor4f x58 = t4::ReluInplace(x57); //features.25
	t4::release(x57);
	t4::tensor4f x59 = t4::Conv2d<3, 3, 1, 1, 1, 1, 1, 1>(x58, ctx.features_26_weight, ctx.features_26_bias); //features.26
	t4::release(x58);
	t4::tensor4f x60 = t4::ReluInplace(x59); //features.27
	t4::release(x59);
	t4::tensor4f x61 = t4::Conv2d<3, 3, 1, 1, 1, 1, 1, 1>(x60, ctx.features_28_weight, ctx.features_28_bias); //features.28
	t4::release(x60);
	t4::tensor4f x62 = t4::ReluInplace(x61); //features.29
	t4::release(x61);
	t4::tensor4f x63 = t4::MaxPool2d<2, 2, 2, 2, 0, 0>(x62); //features.30
	t4::release(x62);
	t4::tensor4f x64 = t4::AveragePool2d<1, 1, 1, 1, 0, 0>(x63); //avgpool
	t4::release(x63);
	t4::tensor0i x65 = t4::Constant<t4::int64>(0);
	t4::tensor1i x66 = t4::Shape(x64);
	t4::tensor0i x67 = t4::Gather(x66, x65);
	t4::release(x66, x65);
	t4::tensor0i x68 = t4::Constant<t4::int64>(-1);
	t4::tensor1i x69 = t4::Unsqueeze<0>(x67);
	t4::release(x67);
	t4::tensor1i x70 = t4::Unsqueeze<0>(x68);
	t4::release(x68);
	t4::tensor1i x71 = t4::Concat<0>(x69, x70);
	t4::release(x69, x70);
	t4::tensor2f x72 = t4::Reshape<2>(x64, x71);
	t4::release(x64, x71);
	t4::tensor2f x73 = t4::Linear(x72, ctx.classifier_0_weight, ctx.classifier_0_bias); //classifier.0
	t4::release(x72);
	t4::tensor2f x74 = t4::ReluInplace(x73); //classifier.2
	t4::release(x73);
	t4::tensor2f x75 = t4::Linear(x74, ctx.classifier_3_weight, ctx.classifier_3_bias); //classifier.3
	t4::release(x74);
	t4::tensor2f x76 = t4::ReluInplace(x75); //classifier.5
	t4::release(x75);
	t4::tensor2f x77 = t4::Linear(x76, ctx.classifier_6_weight, ctx.classifier_6_bias); //classifier.6
	t4::release(x76);
	return x77;
}
